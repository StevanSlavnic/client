...
const Input = (props) => {
	...
	const adornment = () => {
		if (props.config.adornment) {
			const side =
				props.config.adornment.position === 'end' ? 'endAdornment' : 'startAdornment';
			return {
		...
		if (props.config.charCounter) {
			...
			const style = {
				...
				color: props.config.value
					? props.config.value.length > limit ? color.red : color.grey
					: color.grey
			};
			return (
				<span style={style}>{`${props.config.value
					? props.config.value.length
					: 0}/${limit}`}</span>
			);
		}
		return '';
	...
	const selectDisplayRender = props.renderVal
		...
		: props.config.value ? {} : {renderValue: val => <span style={{ color: color.grey }}>{props.config.placeholderStatic}</span>};
	...
	return (
		<TextField
			...
			helperText={[ props.config.error || props.config.infolabel || ' ', CharCounter() ]}
			...
			FormHelperTextProps={{
				style: {
					...
					whiteSpace:
						props.config.group ? 'nowrap' : 'normal',
			...
			InputProps={{
				...
				style:
					props.config.type === 'textarea'
						? { paddingTop: '4px', paddingBottom: '4px', backgroundColor: '#fff' } // textarea input padding
						: {
